...
parser.add_argument("-t", "--threshold", type=float, default=0, help="specify path to .out file")
...
try:
    with open(args.filename, 'r') as file:
        dropped = dropwhile(lambda x: block_start not in x, file) # drop all before block_start
        taken = takewhile(lambda x: block_end not in x, dropped) # drop all after block_end
        # build blocks
        current = ""
        first = False
        for line in taken:
    ...
    for state in states:
        ...
        for trans in state[1]: # transitions
            ...
            if percentage > args.threshold:
                cw.writerow([state[0], trans[0], trans[1], format_decimal(percentage, locale="de")])            
